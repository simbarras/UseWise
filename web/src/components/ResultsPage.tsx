        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-7 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-slate-800">Policy Assistant</p>
                <p className="text-[10px] text-slate-400">Based on the policy, what would you like to know?</p>
              </div>
            </div>
                ...
                messages.map((m, i) => (
                  <div key={i} className={`flex ${m.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div
                      ...
                      style={m.from === 'user' ? { background: 'var(--secondary)' } : {}}
                    >
                      {m.text}
                    </div>
                  </div>
            ...
            {hasAsked && (
              <button
                onClick={() => { setMessages([]); setHasAsked(false); }}
